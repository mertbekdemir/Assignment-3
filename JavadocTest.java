 ...
 * of this testing module.  You should find that when it is compiled,
...
{
    // Windows and Macintosh systems use their own, unique set of 'invisible letters'
    // to visually create a 'carriage-return, line-feed' effect [or, if you prefer, 'enter'].
    // A program needs to know the exact sequence used, and that information can't be hard-coded.
    // Fortunately, this is the kind of information that the Java language always knows for us.
    ...
    
    {
        ...
        ignoredNames.add("CarConstructorTest.java");
        ignoredNames.add("CarLotConstructorTest.java");
        ignoredNames.add("CarLotSpecificationTest.java");
        ignoredNames.add("CarLotTest.java");
        ignoredNames.add("CarSpecificationTest1.java");
        ignoredNames.add("CarSpecificationTest2.java");
        ignoredNames.add("CarTest.java");
        ignoredNames.add("CarTestSuite.java");
        ignoredNames.add("Grader.java");
        ignoredNames.add("JavaDocTest.java");
        ignoredNames.add("SpecificationTest.java");
        ignoredNames.add("TestRunner.java");
    }
    ...
    {
        return name.endsWith(".java") && !(name.startsWith("__SHELL")) && !(ignoredNames.contains(name));        
    }
    ...
    {
        java.util.Stack<String> result = new java.util.Stack<>();
        
        java.util.regex.Matcher m = java.util.regex.Pattern
            .compile(pattern, java.util.regex.Pattern.DOTALL)
            .matcher(s.next());
    ...
    public void evaluateFilesInThisFolder() {        
        ...
        final String DEFAULT_CLASS_HEADER_PENALTY = "-1: The class header for '%s' has been left as the BlueJ-generated default." + NEWLINE_CHAR;
        final String NO_CLASS_HEADER_PENALTY = "-1: The class header for '%s' is missing." + NEWLINE_CHAR;
        final String UNDOCUMENTED_PARAM_PENALTY = "-1: Incorrect javadoc for parameter '%1s', before %2s.%3s()." + NEWLINE_CHAR;
        ...
        for (java.io.File f : new java.io.File(".").listFiles(this))
            try {
                ...
                // @ref http://stackoverflow.com/questions/18901316/curly-quotes-causing-java-scanner-hasnextline-to-be-false-why
                ...
                // (?<=class )\\w+  match a word following 'class', but don't match 'class' itself;
                //                  the word is the name of a class, and does not allow whitespace
                //
                // (?<=interface )\\w+  same explanation as previous, but for an interface
                ...
                // @version Jun24'15,????h  Enumerated types were a structure that I did not anticipate
                ...
                
                {
                    ...
                    // crash with an ArrayIndexOutOfBoundsException if the student omitted
                ...
                // Identify all the methods that are public members of the subject class.  These are the ones we will test.
                ...
                {
                    // Obtain all javadoc for params and returns, or method signatures that could include generics.
                    // The following Regular Expression includes these statements:
                    ...
                    //
                    // public|private|protected     the presence of one of these 'access keywords'
                    ...
                    // Unlikely case.  Should only occur if the student has included a 'static initializer'.
                    if (0 == text.size()) continue;
                    
                    // Separate the method name from its parameter list, using the opening bracket as a delimiter.
                    ...
                    boolean hasReturnValue = true;
                    {
                        ...
                        if (0 > symbolIndex) continue;
                        
                        // Here, we strip the rest of the signature from the method's name.
                        methodName = params[symbolIndex];
                        if (methodName.contains(" void ")) hasReturnValue = false;
                        methodName = methodName.substring(methodName.lastIndexOf(" ") + 1);
                        if (methodName.equals(testedClassName)) hasReturnValue = false;
                       
                        // Don't process any method that isn't a member of the subject class.
                        if (!validMethodNames.contains(methodName)) continue;
                        
                        // Make the types of the parameters into delimiters so the parameter names become isolated.
                        // This RegEx contains the following phrases:
                        //  ?           optional whitespace
                        // (,|^)        has a comma or start-of-input
                        // [^\\s\\<]+   some stuff that isn't whitespace; corresponds to parameter type
                        // (\\<.+\\>)?  optional parameterization of the parameter type; e.g. ArrayList<String>
                        params = params[++symbolIndex].split("( ?(,|^) ?[^\\s\\<]+(\\<.+\\>)? )|\\)");
                    } // end subroutine: break signature into pieces
                
                    // Match each parameter to a @param statement in the javadoc preceding the method.
                    boolean skipFirst = true;
                    paramLoop:
                    for (String param : params)
                    {
                        // The following once-per-loop case was necessitated by the split() command's unwillingness
                        // to omit leading empty-string elements.  It'll exclude tailing ones.
                        if (skipFirst)
                        {
                            skipFirst = false;
                            continue;
                        ...
                        while (i.hasNext())
                        {
                            ...
                            // This condition is true whenever the inspected parameter was documented correctly.
                            if (next.equals("@param " + param))
                            {
                                i.remove();
                                continue paramLoop;
                            }
                        }
                        // If any parameter lacks proper javadocumentation, then the following will occur.
                    ...
                    /*if (0 != text.size())
                        failReport += String.format("-1: %1s.%2s() has at least one @param or @return statement that is incorrect."
                            + "\nThe first word after @param must be the parameter name being described,"
                            + "\nand there can only be one @return statement per method.", testedClassName, methodName) + NEWLINE_CHAR;*/
                } // end loop: method signatures, appearing before block opening-braces
            } // end loop: all java files
            catch (java.io.FileNotFoundException e) {/*technically impossible*/}
            catch (java.lang.ClassNotFoundException e) {
                failReport += String.format("The tester is unable to assess %s.\n> This is OK is it's an interface or abstract class.\n> This is BAD if it's a class file.\n",f.getName());
            } // end catch
